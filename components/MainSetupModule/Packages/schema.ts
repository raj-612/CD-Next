...
export const packageSchema = z.object({
  ...
  if (data.discount_type === 'percentage') {
    return data.discount_percentage !== undefined && data.discount_percentage !== null;
  }
  // If discount type is fixed, require discount_amount
  if (data.discount_type === 'fixed') {
    return data.discount_amount !== undefined && data.discount_amount !== null;
  }
  // If discount type is package, require package_price
  if (data.discount_type === 'package') {
    return data.package_price !== undefined && data.package_price !== null;
  }
  return true;
}, {
